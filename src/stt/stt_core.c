...
ethervox_stt_config_t ethervox_stt_get_default_config(void) {
    ethervox_stt_config_t config = {
        .backend = ETHERVOX_STT_BACKEND_VOSK,
...
int ethervox_stt_process(ethervox_stt_runtime_t* runtime,
    ...
    if (!runtime || !runtime->is_initialized || !runtime->is_processing || 
        !audio_buffer || !result) {
        return -1;
    }
    ...
    for (uint32_t i = 0; i < sample_count && 
         runtime->accumulator_write_pos < runtime->accumulator_size; i++) {
