...
static int extract_smart_home_entities(const char* text, smart_home_command_t* command) {
    ...
    // Action detection
    if (strstr(text_lower, "turn on") || strstr(text_lower, "switch on") || strstr(text_lower, "enable")) {
        strcpy(command->action, "turn_on");
    } else if (strstr(text_lower, "turn off") || strstr(text_lower, "switch off") || strstr(text_lower, "disable")) {
...
static int smart_home_parse_intent(const ethervox_stt_input_t* input, 
    ...
    printf("Smart Home Command: %s %s in %s (value: %.1f)\n",
           command.action, command.device_name, command.room, command.value);
...
ethervox_intent_plugin_t* create_smart_home_plugin(void) {
    ethervox_intent_plugin_t* plugin = (ethervox_intent_plugin_t*)calloc(1, sizeof(ethervox_intent_plugin_t));
    ...
    // Create user data
    smart_home_plugin_data_t* data = (smart_home_plugin_data_t*)calloc(1, sizeof(smart_home_plugin_data_t));
...
int main() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (ret == 0) {
            printf("  Intent: %s (confidence: %.2f)\n", 
                   ethervox_intent_type_to_string(result.type), result.confidence);
            printf("  Entities: %s\n", result.entities);
            printf("  Context: %s\n", result.context);
        } else {
    ...
    printf("  Success rate: %.1f%%\n", 
           ...
           (100.0f * smart_home_plugin->successful_requests / smart_home_plugin->total_requests) : 0.0f);
