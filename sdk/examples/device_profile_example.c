...
static device_template_t device_templates[] = {
    ...
    },
    {
        "Desktop-Development",
        "Desktop",
        "Desktop computer for development and testing",
...
static void print_device_profile(const ethervox_device_profile_t* profile) {
    ...
    printf("  Audio: %d channels @ %d Hz, %d-bit\n",
           profile->mic_array_channels, profile->sample_rate, profile->bit_depth);
    ...
    printf("    Status LED: %d, Recording LED: %d\n", 
           profile->gpio_pins.led_status, profile->gpio_pins.led_recording);
    printf("    Mute Button: %d, Wake Button: %d\n",
           profile->gpio_pins.button_mute, profile->gpio_pins.button_wake);
    printf("    I2C: SDA=%d, SCL=%d\n",
           profile->gpio_pins.i2c_sda, profile->gpio_pins.i2c_scl);
    printf("    SPI: MOSI=%d, MISO=%d, SCLK=%d, CS=%d\n",
           profile->gpio_pins.spi_mosi, profile->gpio_pins.spi_miso,
           profile->gpio_pins.spi_sclk, profile->gpio_pins.spi_cs);
    ...
    printf("    WiFi: %s, Ethernet: %s, Bluetooth: %s\n",
           ...
           profile->has_ethernet ? "Yes" : "No",
           profile->has_bluetooth ? "Yes" : "No");
...
static int save_device_profile_to_file(const ethervox_device_profile_t* profile, const char* filename) {
...
int main() {
    ...
    for (int i = 0; i < 3; i++) {
        printf("  %d. %s (%s)\n     %s\n", 
               ...
               device_templates[i].platform,
               device_templates[i].description);
    }
    printf("\n");
    ...
    printf("| Mic Channels         | %-13d | %-8d | %-11d |\n", 
           profiles[0].mic_array_channels, profiles[1].mic_array_channels, profiles[2].mic_array_channels);
    printf("| Sample Rate          | %-13d | %-8d | %-11d |\n",
           profiles[0].sample_rate, profiles[1].sample_rate, profiles[2].sample_rate);
    ...
    printf("| Max Concurrent       | %-13d | %-8d | %-11d |\n",
           profiles[0].max_concurrent_streams, profiles[1].max_concurrent_streams, profiles[2].max_concurrent_streams);
    ...
    printf("\nDevice profiles demonstrate how EtherVox can be adapted to different hardware configurations\n");
