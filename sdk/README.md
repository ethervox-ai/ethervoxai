...
1. **Optimize for performance** - Intent parsing should complete in < 100ms
1. **Support multiple languages** - Use standardized language codes (ISO 639-1)
1. **Provide detailed statistics** - Track success rates and processing times
1. **Use consistent naming** - Follow naming conventions for consistency

## Model Routing

1. **Implement intelligent selection** - Route based on complexity, latency, cost
1. **Add fallback mechanisms** - Handle model failures gracefully  
1. **Monitor performance** - Track response times and success rates
1. **Cache responses** - Implement caching for repeated requests
1. **Respect rate limits** - Handle API rate limiting appropriately

## Device Profiles

1. **Match hardware capabilities** - Configure settings appropriate for platform
1. **Optimize for power** - Use appropriate sleep timeouts and power management
1. **Document pin assignments** - Clearly specify GPIO usage
1. **Test thoroughly** - Validate configuration on target hardware
1. **Version configurations** - Track profile versions for compatibility
...
1. **Use local models** - For low-latency requirements
1. **Implement request caching** - Cache frequent responses  
1. **Optimize parsing algorithms** - Use efficient pattern matching
1. **Monitor resource usage** - Track CPU and memory consumption
