...
async function runDemo() {
    ...
    const ai = new index_1.EthervoxAI({
    ...
    const languages = index_1.multilingualRuntime.getSupportedLanguages();
    console.log(`Supported languages: ${languages.join(', ')}`);
    const profiles = index_1.multilingualRuntime.getLanguageProfiles();
    profiles.forEach(profile => {
        console.log(`  - ${profile.name} (${profile.code})${profile.isDefault ? ' [DEFAULT]' : ''}`);
    });
    // 3. Test Local LLM Stack
    console.log('\n🧠 Testing Local LLM Stack...');
    const models = index_1.localLLMStack.getLocalModels();
    console.log(`Available models: ${models.map(m => m.name).join(', ')}`);
    const currentModel = index_1.localLLMStack.getCurrentModel();
    ...
    const privacySettings = index_1.privacyDashboard.getPrivacySettings();
    ...
    const cloudQueries = index_1.privacyDashboard.getCloudQueryHistory(5);
    console.log(`\nRecent cloud queries: ${cloudQueries.length}`);
    const deviceStatuses = index_1.privacyDashboard.getDeviceStatuses();
    ...
    console.log('\n📦 Testing Data Export...');
    const userData = index_1.privacyDashboard.exportUserData();
